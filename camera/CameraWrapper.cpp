...
static int camera_set_torch_mode(int camera_id, int enabled);
...
camera_module_t HAL_MODULE_INFO_SYM = {
    .common = {
         ...
         .name = "Moorefield Camera Wrapper",
...
char * camera_fixup_setparams(struct camera_device * device, const char * settings)
{
    ...
    ALOGI("%s: original parameters for camera %d", __FUNCTION__, CAMERA_ID(device));
    params.dump();
#endif
...
#ifdef LOG_PARAMETERS
    ALOGI("%s: fixed parameters for camera %d:", __FUNCTION__, id);
    params.dump();
#endif
    return ret;
}
...
{
    ALOGI("%s camera %d", __FUNCTION__, camera_id);
    if (check_vendor_module())
        return 0;
